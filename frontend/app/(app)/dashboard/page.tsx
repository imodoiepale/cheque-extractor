...
import { Upload, FileCheck, CheckCircle, Download } from 'lucide-react';
...
export default async function DashboardPage() {
  ...
  const { count: pendingReview } = await supabase
    .from('checks')
    .select('*', { count: 'exact', head: true })
    .in('status', ['review_required', 'review_suggested']);
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your check processing</p>
        </div>
        <Link
          href="/upload"
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        <CheckList checks={checks as Check[] || []} />
      </div>
    </div>
  );
}
